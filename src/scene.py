...
class SceneParser:
    ...
    def __load(self, filepath):
        """Loads the raw JSON data from the scene file."""
        try:
    ...
    def __validate(self, data):
        """Validates the scene structure, keys, tile map, and sprite constraints."""
        # 1. Validate required top-level keys: transparent_index, tile_map, sprites
        ...
        # 2. Validate transparent_index (must be an integer between 0 and 15)
        ...
        # 3. Validate tile_map (must be 15 rows and 20 columns)
        ...
        # 4. Validate sprites (must be a list with specific fields)
